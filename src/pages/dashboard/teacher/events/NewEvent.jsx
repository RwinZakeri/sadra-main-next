...
//     const [imagePath, setImagePath5] = useState(''); //18
...
//             try {
//                 const responseToken = await axios.get('http://localhost:3001/api/dashboard/whoami', { withCredentials: true });
...
//             // formData.append('files', imageData2);
...
//                     },
//                 });
//                 imagePath5 = await result.data.path
//             } catch (error) {
//                 console.error(error);
//             }

//                 // imagePath15 = await result?.data.paths[0].split('\\').join('/')

//             // const imagePath2 = await result?.data.paths[1].split('\\').join('/');

//             // Set image paths in state
//             // setImagePath5(imagePath1);
//             // setImagePath2(imagePath2);
...
function NewEvent() {
  return (
    <div>
      <h1>این صفحه در حال حاضر موجود نمیباشد..</h1>
    </div>
  );
}
